namespace NeoNetsphere.Network.Services
{
    ...
    internal class ClubService : ProudMessageHandler
    {
        ...
        {
            session.SendAsync((object)new ClubJoinWaiterInfoAckMessage());
        }
        ...
        public void ClubSearchReq(GameSession session, ClubSearchReqMessage message)
        {
            ...
            ClubSearchAckMessage searchAckMessage = new ClubSearchAckMessage();
            // todo searchAckMessage.set_Unk1(1);
            gameSession.SendAsync((object)searchAckMessage);
        }
        ...
        {
            if (session.Player == (Player)null)
                return;
            session.SendAsync((object)new ClubRankListAckMessage());
        }

        [MessageHandler(typeof(ClubAddressReqMessage))]
        public void CClubAddressReq(GameSession session, ClubAddressReqMessage message)
        {
            session.SendAsync((object)new ClubAddressAckMessage("", 0U));
        }
        ...
        public void ClubNoteSendReq2(ChatSession session, ClubNoteSendReq2Message message)
        {
            ...
            if (gameSession == null)
                return;
            ClubNoteSendAckMessage noteSendAckMessage = new ClubNoteSendAckMessage();
            //noteSendAckMessage.set_Unk(1); todo
            gameSession.SendAsync((object)noteSendAckMessage);
        }


        [MessageHandler(typeof(ClubNoticePointRefreshReqMessage))]
        public void ClubNoticePointRefreshReq(
      GameSession session,
      ClubNoticePointRefreshReqMessage message)
        {
            session.SendAsync((object)new ClubNoticePointRefreshAckMessage());
        }

        [MessageHandler(typeof(ClubNoticeRecordRefreshReqMessage))]
        public void ClubNoticeRecordRefreshReq(
        GameSession session,
        ClubNoticeRecordRefreshReqMessage message)
        {
            session.SendAsync((object)new ClubNoticeRecordRefreshAckMessage());
        }
        ...
        {
            await this.ClubUnjoinReq2(session, message.Map<ClubUnjoinReqMessage, ClubUnjoinReq2Message>());
        }
